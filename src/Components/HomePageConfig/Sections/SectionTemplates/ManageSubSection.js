...
function ManageSubSection({
  subSection,
  updateSubSection,
  setVisibleSubSection,
  type,
}) {
  const MIN_IMAGES = type === HOME_PAGE_SECTION_TYPES.scroll ? 1 : 6;
  const { images } = subSection;
  ...
  const actionBodyTemplate = (rowData) => {
    return (
      <React.Fragment>
        <Button
          disabled={editImages.length <= MIN_IMAGES}
          icon="pi pi-trash"
      ...
      >
        ...
        <div className="p-jc-between p-d-flex p-mt-2">
          <Button
            ...
            label="Cancel"
            onClick={() => setVisibleSubSection(false)}
          />
          <Button
            className="p-py-1"
            label="Save"
...
export default ManageSubSection;
